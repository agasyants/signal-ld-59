class_name RingObstacle
extends Obstacle

var gap_size: float = PI * 0.5  # размер дырки — четверть окружности
var sides: int = 16
...
func _build_mesh(tunnel_radius: float):
	...
	for i in sides:
		...
		var b = step * (i + 1)
		if a < gap_size:
			continue  # пропускаем дырку (rotation.z уже повернул)
...
func _build_collider(tunnel_radius: float):
	...
	for i in sides:
		...
		shape.position = Vector3(cos(mid) * (inner + (tunnel_radius - inner) * 0.5),
		sin(mid) * (inner + (tunnel_radius - inner) * 0.5), -0.25)
